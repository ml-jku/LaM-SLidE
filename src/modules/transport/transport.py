...
class ModelType(enum.Enum):
    ...
    DATA = enum.auto()  # the model predicts x
...
class Transport:
    ...
    def get_score(
        ...
        if self.model_type == ModelType.NOISE:
            ...
            score_fn = lambda x, t, model, **kwargs: self.path_sampler.get_score_from_data(
                model(x, t, **kwargs), x, t
            )
        else:
            raise NotImplementedError()
...
class Sampler:
        ...
        *,
        ...
        def _likelihood_drift(x, t, model, **model_kwargs):
            ...
            with th.enable_grad():
                ...
                grad = th.autograd.grad(th.sum(self.drift(x, t, model, **model_kwargs) * eps), x)[
                    0
                ]
    ...
    def get_sample_fn(
        ...
        if sampling_method == "SDE":
            sde_kwargs.update(sampling_kwargs)
        elif sampling_method == "ODE":
            ode_kwargs.update(sampling_kwargs)

        if sampling_method == "SDE":
            return self.sample_sde(**sde_kwargs)
        elif sampling_method == "ODE":
            return self.sample_ode(**ode_kwargs)
