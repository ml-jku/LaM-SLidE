...
import src.modules.protein as protein
...
def frames_torsions_to_atom14(frames: Rigid, torsions: torch.Tensor, aatype: torch.Tensor):
...
def atom37_to_torsions(all_atom_positions, aatype, all_atom_mask=None):
    ...
    if all_atom_mask is None:
        all_atom_mask = torch.from_numpy(rc.RESTYPE_ATOM37_MASK[aatype]).to(aatype.device)
